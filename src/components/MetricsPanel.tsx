        ...
        <div className={`space-y-2 text-sm font-mono rounded-lg p-3 ${isDark ? 'bg-black/20' : 'bg-slate-50'}`}>
          ...
          <div className={`flex justify-between items-center pt-2 mt-2 border-t ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
            <span className="flex items-center gap-2">
              ...
              <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Fixed Point x*</span>
            </span>
            <span className="text-yellow-500">{fixedPoint.toFixed(8)}</span>
          </div>
        </div>
      </div>
...
function ResidualChart({ results, enabledAlgorithms, currentStep, isDark }: ResidualChartProps) {
  const enabledAlgos = ALGORITHMS.filter(a => enabledAlgorithms.has(a.id));
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Y-axis labels with proper superscripts */}
        {yTicks.map(logVal => (
          <g key={`y-${logVal}`}>
            <line
              x1={padding.left}
